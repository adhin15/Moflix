...
import { getDetailMovie, getMovieCastingList, getMovieKeywords, getExternalIds } from "../../services/Getdata";
import { useEffect, useState } from "react";
import Layout from "../../components/Layout";
import CastingList from "../../components/CastingList";
import CardSkeleton from "../../components/skeleton/CardSkeleton";
import DetailSkeleton from "../../components/skeleton/DetailSkeleton";
import { UsdFormatter } from "../../services/Helper";
...
const DetailMovie = () => {
  ...
  useEffect(() => {
    if (id) {
      ...
      getDetailMovie(query).then((resDetail) => {
        ...
        getMovieCastingList(query).then((resCast) => {
          setCastingList(resCast);
          getMovieKeywords(query).then((resKeyword) => {
            setMovieKeywords(resKeyword);
            getExternalIds(query).then((resExternalIds) => {
              setExternalIds(resExternalIds);
              console.log("log external id", resExternalIds);
            });
            setIsLoading(false);
          });
        });
  ...
  useEffect(() => {
    if (detailMovie && !isLoading) {
      console.log("log detail", detailMovie);
      circle();
    }
    ...
    <>
      <Head>
       ...
       #bg-cover{
        &:before{
            z-index:-1;
            background-image:url(https://image.tmdb.org/t/p/original${detailMovie?.backdrop_path});
        }
       }
       svg{
      ...
      <Layout>
        <div className="container w-full max-w-full pt-5">
          <div
            id="bg-cover"
            className={`flex before:bg-cover before:bg-center before:bg-no-repeat before:w-full w-full before:absolute before:inset-0 before:max-h-[35rem] max-h-[35rem] before:opacity-30 mb-[44px]`}
          >
            {!isLoading ? (
              <>
                <div
                ...
                <div className={`w-2/3 max-h-full rounded-lg overflow-hidden px-8 flex flex-col justify-center`}>
                  <h2 className="text-4xl my-2" data-aos="fade-left" data-aos-delay="300">
                    <span className="font-bold">{detailMovie?.title}</span>(
                    {detailMovie?.release_date.slice(0, 4) || "-"})
                  </h2>

                  <div className="my-2" data-aos="fade-left" data-aos-delay="400">
                    <span>
                      {detailMovie?.release_date} ({detailMovie?.production_countries[0]?.iso_3166_1})
                    </span>
                    <span>
                      <svg
                  ...
                  <div className="flex items-center	my-2" data-aos="fade-left" data-aos-delay="500">
                    <span className="relative " style={{ left: 0, height: 56, width: 56 }}>
                      <p className="text-[16px] py-4 px-3 absolute" style={{ left: 2, bottom: 0, zIndex: 1 }}>
                        {detailMovie?.vote_average.toString().slice(0, 3) * 10}%
                      </p>
          ...
          <div className="flex">
              ...
              <div>
                {!isLoading ? (
                  <div className={`relative ${castingScrolled ? "scrolled" : ""}`} id="cast_scroller">
                    ...
                    <Link href={`/cast/movies-${id}`}>Full Cast & Crew</Link>
                  </div>
                ) : (
            ...
            <div className="w-1/3 min-w-[260px] px-3" data-aos="fade-up" data-aos-delay="300">
              <div className="h-full">
                ...
                <div className="mb-8">
                  ...
                  <span className="px-2">
                    {detailMovie?.homepage && (
                      <Link href={`${detailMovie?.homepage}`}>
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width="20"
                          height="20"
                ...
                <div className="w-full mb-4">
                  ...
                  <p className="text-[16px]">{detailMovie?.status}</p>
                </div>
                <div className="w-full mb-4">
                  <h4 className="text-[16px] font-bold">Original Language</h4>
                  <p className="text-[16px]">{detailMovie?.original_language.toUpperCase()}</p>
                </div>
                <div className="w-full mb-4">
